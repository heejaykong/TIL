...
__1. countItems_procedure.sql__
```sql
...
--  상수 선언
	...
	DECLARE VALUE__ITEM_STATUS__AWAITING VARCHAR(50);
	...
	SET VALUE__ITEM_STATUS__AWAITING = 'AWAITING';
	...
	IF _itemStatus IS NOT NULL THEN
		...
		END IF;
		IF _itemStatus = VALUE__ITEM_STATUS__AWAITING THEN
			SET ALONE_ITEMS_QUERY = CONCAT(ALONE_ITEMS_QUERY, 'AND item.item_status = 5 ');
		END IF;
	END IF;
	...
	IF _itemStatus IS NOT NULL THEN
		...
		END IF;
		IF _itemStatus = VALUE__ITEM_STATUS__AWAITING THEN
			SET BOUND_ITEMS_QUERY = CONCAT(BOUND_ITEMS_QUERY, 'AND item.item_status = 5 ');
		END IF;
	END IF;
